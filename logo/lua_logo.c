/*
 * Rozšíření interpreteru jazyka Lua o příkazy "left", "right", "forward",
 * "back", "home", "clean", "penup" a "pendown", pomocí nichž lze kreslit různé
...
/*
 * Vytvoření bitmapy s 8bpp (stupně šedi)
 */
...
/*
 * Vykreslení jednoho pixelu do bitmapy. Vždy je nastavena černá barva na bílém
 * pozadí.
 */
...
/*
 * Vykreslení úsečky Bresenhamovým algoritmem
 */
void bitmapLine(Bitmap *bitmap, int x1, int y1, int x2, int y2)
{
    ...
    if (x1>x2)
    { // otočení x-ových souřadnic
        xinc1=-xinc1;
        xinc2=-xinc2;
    }
    if (y1>y2)
    {  // otočení y-ových souřadnic
        yinc1=-yinc1;
        yinc2=-yinc2;
    }
...
/*
 * Návrat želvy k počátku s vymazáním obrazovky
 */
